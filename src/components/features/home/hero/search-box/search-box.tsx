...
export const SearchBox = ({ className, industries }: SearchBoxProps) => {
	...
	return (
		<form
			className={cn(
				"bg-white flex gap-4 shadow-[0_18px_40px_rgba(25,15,9,.1)] rounded-lg p-3",
				className
			)}
			onSubmit={onSubmit}
		>
			<select
				...
				className="outline-none border-none  text-js-grey-100 text-sm bg-[url('/industry.svg')] bg-no-repeat pl-8"
			...
			<select
				name="location"
				className="outline-none border-none bg-none text-js-grey-100 text-sm "
